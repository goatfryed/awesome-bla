        ...
        <div className="container">
            <article className="media">
                ...
                <div className="media-content">
                    <strong>
                        {bucketList.title}({bucketList.id})
                    </strong>
                    <br />
                    <small>
                        {bucketList.voteCount}· <a onClick={upvoteList}>Like</a>· <a onClick={downvoteList}>Dislike</a>· <span>{moment(bucketList.created).fromNow()}</span>·{" "}
...
function BucketListComments({ bucketList, update }) {
    async function addCommentToBucketList(comment) {
        await backendFetch.post("/bucketlists/" + bucketList.id + "/comments/", { body: JSON.stringify(comment) });
        update();
    }

    return <CommentsBlock onRootCommentCreation={addCommentToBucketList} onReplyCreated={update} comments={bucketList.comments} />;
}
...
const NavTab = withRouter(({ to, location, children }) => {
    return (
        <li className={location.pathname.startsWith(to) ? "is-active" : null}>
